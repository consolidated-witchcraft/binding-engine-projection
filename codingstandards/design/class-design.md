...
* You MUST use private constants over inline comparisons.
* Pure functions MUST be static.
* If a class can be readonly, it SHOULD be.

__Don't do this:__
```php
    public function addItemsToBasket(int $qty): void {
        if ($qty > 50) {
            throw new TooManyItemsAddedToBasketException(
                message: "You may not add more than fifty items to your basket at a time."
            );
        }
    }
```
